import "@trestleinc/replicate/client";
import "./api.js";
import { clsx as clsx$1 } from "clsx";
import { BaseQueryBuilder, createLiveQueryCollection } from "@tanstack/db";
import { A as ATTACHMENT_KEY, o as lifecycle_function_unavailable, f as attributes, g as clsx, n as ensure_array_like, q as element, k as derived, e as attr, l as attr_class, t as attr_style, m as stringify } from "./index2.js";
import { a as ssr_context, h as hasContext, g as getContext, s as setContext, b as getAllContexts, e as escape_html } from "./context.js";
...
function useLiveQuery(configOrQueryOrCollection, deps = []) {
  const collection = (() => {
    ...
    if (typeof unwrappedParam === `function`) {
      ...
      return createLiveQueryCollection({ query: unwrappedParam, startSync: true });
    } else {
      return createLiveQueryCollection({ ...unwrappedParam, startSync: true });
    }
  })();
  const state = new SvelteMap();
  let internalData = [];
  let status = collection ? collection.status : `disabled`;
  return {
    get state() {
      return state;
    },
...
function StatusIcon($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    const { status, size = 14, class: className = "" } = $$props;
    const statusColors = {
      backlog: "#6b7280",
      // gray-500
      todo: "#3b82f6",
    ...
    const fillOpacity = status === "todo" ? 0.15 : 0;
    if (status === "done") {
      $$renderer2.push("<!--[-->");
      $$renderer2.push(`<svg${attr("width", size)}${attr("height", size)} viewBox="0 0 16 16" fill="none"${attr_class(clsx(className))} role="img"${attr("aria-label", label)}><title>${escape_html(label)}</title><circle cx="8" cy="8" r="7"${attr("stroke", color)} stroke-width="1.5"${attr("fill", color)}></circle><path d="M5 8l2 2 4-4" stroke="white" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path></svg>`);
    } else {
      $$renderer2.push("<!--[!-->");
      if (status === "canceled") {
        $$renderer2.push("<!--[-->");
        $$renderer2.push(`<svg${attr("width", size)}${attr("height", size)} viewBox="0 0 16 16" fill="none"${attr_class(clsx(className))} role="img"${attr("aria-label", label)}><title>${escape_html(label)}</title><circle cx="8" cy="8" r="7"${attr("stroke", color)} stroke-width="1.5"></circle><path d="M5 8h6"${attr("stroke", color)} stroke-width="1.5" stroke-linecap="round"></path></svg>`);
      } else {
        $$renderer2.push("<!--[!-->");
        if (status === "in_progress") {
          $$renderer2.push("<!--[-->");
          $$renderer2.push(`<svg${attr("width", size)}${attr("height", size)} viewBox="0 0 16 16" fill="none"${attr_class(clsx(className))} role="img"${attr("aria-label", label)}><title>${escape_html(label)}</title><circle cx="8" cy="8" r="7"${attr("stroke", color)} stroke-width="1.5"></circle><path d="M8 1a7 7 0 0 1 0 14"${attr("fill", color)}${attr("stroke", color)} stroke-width="1.5"></path></svg>`);
        } else {
          $$renderer2.push("<!--[!-->");
          $$renderer2.push(`<svg${attr("width", size)}${attr("height", size)} viewBox="0 0 16 16" fill="none"${attr_class(clsx(className))} role="img"${attr("aria-label", label)}><title>${escape_html(label)}</title><circle cx="8" cy="8" r="7"${attr("stroke", color)} stroke-width="1.5"${attr("fill", color)}${attr("fill-opacity", fillOpacity)}></circle></svg>`);
        }
        $$renderer2.push(`<!--]-->`);
      }
      $$renderer2.push(`<!--]-->`);
...
function PriorityIcon($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    const { priority, size = 14, class: className = "" } = $$props;
    ...
    const filledBars = priority === "urgent" ? 4 : priority === "high" ? 3 : priority === "medium" ? 2 : priority === "low" ? 1 : 0;
    const containerSize = size + 6;
    $$renderer2.push(`<div${attr_class(clsx(cn("inline-flex items-center justify-center rounded-sm bg-border", className)))}${attr_style(`width: ${stringify(containerSize)}px; height: ${stringify(containerSize)}px;`)}><svg${attr("width", size)}${attr("height", size)} viewBox="0 0 16 16" fill="none" role="img"${attr("aria-label", label)}><title>${escape_html(label)}</title><!--[-->`);
    const each_array = ensure_array_like([0, 1, 2, 3]);
    for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
      let i = each_array[$$index];
      $$renderer2.push(`<rect${attr("x", 1 + i * 4)}${attr("y", 12 - (i + 1) * 2.5)}${attr("width", 3)}${attr("height", (i + 1) * 2.5)}${attr("rx", 0.5)}${attr("fill", i < filledBars ? color : "var(--border)")}></rect>`);
    }
    $$renderer2.push(`<!--]--></svg></div>`);
  });
}
export {
  boolToEmptyStrOrUndef as A,
  onDestroy as B,
  Context as C,
  Dismissible_layer as D,
  Escape_layer as E,
  Focus_scope as F,
  createSubscriber as G,
  defaultWindow as H,
  Icon as I,
  boxFrom as J,
  cssToStyleObj as K,
  ...
  styleToString as M,
  getWindow as N,
  MenuRadioItemState as O,
  Portal as P,
  MenuRadioGroupState as Q,
  MenuRootState as R,
  Scroll_lock as S,
  Text_selection_layer as T,
  MenuMenuState as U,
  MenuContentState as V,
  DropdownMenuTriggerState as W,
  createId as a,
  boxWith as b,
  cn as c,
  attachRef as d,
  ...
  afterSleep as t,
  useId as u,
  PresenceManager as v,
  watch as w,
  SPACE as x,
  ENTER as y,
  getDataOpenClosed as z
};
